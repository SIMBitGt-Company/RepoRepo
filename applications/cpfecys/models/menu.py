...
response.meta.author = 'Boris Aguilar <me@borisaguilar.com> & Omar Vides <omarvides@gmail.com> & William Abdalla <wil.fuentes005@gmail.com> & Gustavo Vega <lecatavo@gmail.com> & Carlos Rodas <carlos8.r@gmail.com>'
...
response.menu.extend([(T('About Us'), False, URL('default','about_us'), [])])
    ...
    response.menu.extend([
        ...
        (T('Assignation'), False, URL(),[
         ...
         (T('Academic Control'), False, URL(), [
            ...
            ((T('Periodos Variables'), False, URL(), [               
                (T('Periodos Variables'), False, URL('activity_control','periodo_variable'), [])
         ...
         (T('Practice Reports'), False, URL(),[
            ...
            ((T('Items'), False, URL(), [
                ...
                (T('Delivered Download'), False, URL('admin', 'delivered_download_button/type'), [])
            ])),
        ...
        (T('Evaluations')+" 360", False, URL(), [
            ...
            ((T('Evaluation results'), False, URL(), [    
                (T('Resultados Generales'), False, URL('evaluation','results_search'), []),
                (T('Gráficas Generales'), False, URL('evaluation','graph'), []),
                (T('Busqueda Individual'), False, URL('evaluation','search'), []),
                (T('History Evaluations'), False, URL('management_reports','evaluation_result'), [])
            ]))
        ]),

          #emarquez, se agrego el menu de periodo variable
         ...
         (T('Configuration'), False, URL(),[
            ...
            ((T('Academic Control Parameters'), False, URL(), [
                ...
                (T('Period'), False, URL('activity_control','student_control_period'), []),
                 (T('Periodo Variable'), False, URL('activity_control','periodo_variable'), [])
            ])),
...
#RGUARAN: Se comentan líneas para mejorar rendimiento, cont_news = contador de correos no leidos en menu
...
#CERODAS 2: ADDING NEW MENU ADMIN TESIS
#if auth.has_membership(role="Tesis-Administrator") or auth.has_membership(role="Super-Administrator"):
if auth.has_membership(role="Tesis-Administrator"):
    response.menu.extend([
        (T('Tesis'), False, URL(),[
            ((T('Gestión de Tesis'), False, URL(), [
                (T('Estados'), False, URL('tesis','status'), []),
                (T('Modalidades'), False, URL('tesis','modality'), []),
                (T('Areas de Investigacion'), False, URL('tesis','research_areas'), []),
                (T('Tutores'), False, URL('tesis','tutor'), []),
                (T('Asesores'), False, URL('tesis','adviser'), []),
            ])),
            (T('Temas de Tesis'), False, URL('tesis','asign_topics'), []),
            (T('Consultas'), False, URL('tesis', 'reports'), []),
            ])
])

#CERODAS 2: ADDING NEW MENU STUDENT TESIS
if auth.has_membership(role="Academic"):
    response.menu.extend([
    (T('Tesis'), False, URL(),[
        (T('Temas de Tesis'), False, URL('tesis', 'reports_student'), [])
        ])
    ])


#CERODAS 2: ADDING NEW MENU STUDENT EPS
#se agrega el menu de eps si y solo si tiene algun proyecto asignado
if auth.has_membership(role="Academic"):
    assign_eps = db(db.eps_assignment.student == auth.user.username).select().first();
    if assign_eps:
        response.menu.extend([
        (T('Eps'), False, URL(),[
            (T('Calendario'), False, URL('eps', 'calendar_student'), [])
            ])
        ])


#CERODAS 2: ADDING NEW MENU ADMIN EPS
#if auth.has_membership(role="EPS-Administrator") or auth.has_membership(role="Super-Administrator"):
if auth.has_membership(role="EPS-Administrator"):
    response.menu.extend([
        (T('Eps'), False, URL(),[
            ((T('Gestión de EPS'), False, URL(), [
                (T('Estados'), False, URL('eps','status'), []),
                (T('Ciclos'), False, URL('eps','period'), []),
                (T('Modalidades'), False, URL('eps','modality'), []),
                (T('Actividades'), False, URL('eps','activity'), []),
                (T('Períodos'), False, URL('eps','year'), []),
                (T('Instituciones'), False, URL('eps','institution'), []),
            ])),
            (T('Asignar Estudiantes'), False, URL('eps','asign_students'), []),
            (T('Agendar Actividades'), False, URL('eps', 'calendar_activity'), []),
            (T('Seguimiento'), False, URL(), [
                (T('Actividades Digitales'), False, URL('eps','tracking_activity'), []),
                (T('Entregables'), False, URL('eps','tracking'), []),
            ]),
            (T('Email'), False, URL('eps','send_email'), []),
            (T('Consultas'), False, URL('eps', 'reports'), []),
            ])
])
...
#RGUARAN:se agrega menu para resultados pruebas 360
    response.menu.extend([ (T('Evaluations')+" 360", False, URL(), [
            (T('Resultados Generales'), False, URL('evaluation','results_search'), []),
            (T('Gráficas Generales'), False, URL('evaluation','graph'), []),
            (T('Busqueda Individual'), False, URL('evaluation','search'), [])
        ]),
...
#emarquez: separacion de menus, semestre-escuela
if auth.has_membership(role="Student"):
    
    response.menu.extend([ (T('Proyectos'), False, URL(), [
            (T('Final Practice'), False, URL('student','index'), []),
            (T('Escuela Vacaciones'), False, URL('student','periods'), [])
        ]),
    ])
