...
if not request.env.web2py_runtime_gae:
    ...
    db3 = DAL('mysql://root@localhost/cpfecys_scheduler',pool_size=1,check_reserved=['all'],fake_migrate=True)
else:
...
def send_evaluation_notification_result():
    ...
    print '------------Metodo para el correo de los resultados de las evaluaciones'
    email_list = []
    if  evaluation is not None:
        for person in db((db.evaluation_result.period == cperiod.id)).select(groupby=db.evaluation_result.evaluated):
            ...
            rows=db((db.evaluation_solve_detail.evaluation_result == db.evaluation_result.id)&
                     ...
                     (db.auth_user.id == person.evaluated)&
                     (db.evaluation_result.period == cperiod.id)).select(db.question_repository.question,columnTotal,groupby=db.question_repository.id)
            suma = Decimal(0.0)
            promedio = 0
