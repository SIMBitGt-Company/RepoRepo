...
def academic_assignation():
    ...
    if update_form.accepts(request.vars,formname='update_form'):
        try:
            ...
            result = db(db.auth_membership.user_id==user.id).select()
            roll_var = ''
            i = 0;
            for a in result:
                if i == 0:
            ...
            if update_form.vars.delete_check:
                ...
                if db(db.grades.academic_assignation==update_form.vars.assignation_id).select().first() is None:
                    ...
                    db.academic_course_assignation_log.insert(user_name = user.username, 
                                                            roll =  str(roll_var), 
                                                            operation_log = 'delete', 
    ...
    if ((currentyear_period.id == cpfecys.current_year_period().id)&(permition_var == True)):
        ...
        print '### check.project ', check.project
        checkProject = db((db.user_project.assigned_user==user.id)&\
        ...
        print '### checkProject ', checkProject
        b=0
    ...
    if cpfecys.is_semestre(request.vars['year_period']):
        ...
        periods_after = db((db.period_year.period=='1')| (db.period_year.period=='2')).select(limitby=(currentyear_period.id, \
            end_index))
    ...
    if cpfecys.is_semestre(request.vars['year_period'])==False:
        var_period = db((db.period.id == db.period_year.period)&(db.period_year.id == request.vars['year_period'])).select().first()
...
def academic_assignation_upload():
    ...
    if request.vars.csvfile != None:
        try:
            ...
            return dict(success = False,
                ...
                name=name, files=files,date_finish=date_finish)
        try:
            cr = csv.reader(file,dialect=csv.excel_tab, delimiter=',', quotechar='"')
	    #cr = csv.reader(open(file,'rU'),delimiter=',',quotechar='"')
            success = True
            header = next(cr)
            #response.flash = ('error header')
	    #return dict(success=False, file=False, periods=periods, current_period=current_period,name=name,files=files,date_finish=date_finish)
	    #header=next(cr)
            for row in cr:
                ## parameters
                rcarnet = row[0]
                ...
                if usr is None:
                    ...
                    if rcarnet == '':
                        ...
                        if rlaboratorio != 'T' and rlaboratorio != 'F':
                            ...
                            #Add log
                            db.academic_log.insert(user_name = user.username, 
                                                    roll =  'Student', 
                                                    operation_log = 'insert', 
                            ...
                            for a in result:
                                if i == 0:
                                    ...
                                    roll_var = roll_var + ',' + a.group_id.role
                            db.academic_course_assignation_log.insert(user_name = user.username, roll =  roll_var, 
                                                    operation_log = 'insert', 
                                                    after_carnet = rcarnet, 
    ...
    if auth.has_membership('Student'):
        ...
        if cpfecys.is_semestre(request.vars['year_period']):
            ...
            control_period = db(db.period_detail.period==current_period.period).select().first()
            try:
                date_finish = control_period.date_finish
                if  ( (datetime.datetime.now() < control_period.date_start) | (datetime.datetime.now() > control_period.date_finish) ):
                    except_var = False
