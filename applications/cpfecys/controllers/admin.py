...
def activities_report_tutor(report):
    ...
    if report.assignation.project.area_level.name=='DTT Tutor Académico' and (report.status.name=='Draft' or report.status.name=='Recheck'):
        ...
        # cambio rome ## activitiesGrades = db(db.grades).select(db.grades.activity.with_alias('id'), distinct=True)
...
def assignations():
    ...
    #emarquez: comentado, que no dependa de periodos estaticos
    ...
    #emarquez , comentado para no depender de periodos estaticos
...
def GetPeriodosVariables():
    return response.json(db((db.period.id==db.period_detail.period)&\
    (db.period_year.period==db.period.id)).select())


def GetPeriodosSemestre():    

    listperios = db(db.period_detail.period)._select(db.period_detail.period)

    return response.json(db((~db.period.id.belongs(listperios))&(db.period_year.period==db.period.id)).select(orderby=~db.period_year.id))
    #return response.json(db((db.period.id<>db.period_detail.period)&\
    #(db.period_year.period==db.period.id)).select(orderby=~db.period_year.yearp))
