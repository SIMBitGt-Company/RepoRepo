...
def courses_list():
            ...
            if auth.has_membership('Student') or auth.has_membership('Teacher'):
                try:
                    ...
                    if db((db.user_project.assigned_user==auth.user.id)&\
                        ...
                        (db.user_project.period != db.period_detail.period)&\
                    ...
                    if db((db.user_project.assigned_user==auth.user.id ) &\
                         (db.user_project.period == db.period_year.id)&\
                         (db.user_project.period == period_temp.id) & \
                         (db.user_project.period==db.period_detail.period)).select(db.user_project.ALL).first() is not None:
    ...
    if (auth.has_membership('Super-Administrator') or (auth.has_membership('Ecys-Administrator')& (ecys_var == True)) ):
        ...
        for course in db((db.project.area_level==area.id)&(db.user_project.project==db.project.id)&(db.user_project.period == db.period_year.id)&((db.user_project.period <= period.id)&((db.user_project.period + db.user_project.periods) > period.id))).select(db.project.ALL, orderby=db.project.name, distinct=True):
            ...
            queryLab = 'select obtenerPromedio(\'T\',' + str(course.id) + ',' + str(period.id) +') as promedio;'
            queryCurso = 'select obtenerPromedio(\'F\',' + str(course.id) + ',' + str(period.id) +') as promedio;'
            avgLab = db.executesql(queryLab ,as_dict=True)
            avgCurso = db.executesql(queryCurso ,as_dict=True)
            average_laboratory = avgLab[0]['promedio']
            average_class = avgCurso[0]['promedio']
            
            if average_laboratory is None:
                average_laboratory = 0
        ...
        if (db(db.period_detail.period==period.id).select()):
            ...
            semestreAnterior= db.executesql('SELECT max(id) max from period_year where id< ' + str(period.id) +' and id not in( select period from period_detail);')

            #Ahora obtener el semestre anterior , seleccionando los que tengan 2 periodos
            coursesAdminAnterior =  db((db.user_project.assigned_user==auth.user.id)&\
                            (db.user_project.period == db.period_year.id)&\
                            (db.user_project.period == semestreAnterior[0][0]) & \
                            (db.user_project.periods == 2) & \
                            (db.user_project.project==db.project.id)&\
                            (db.project.area_level==area.id) ).select()

            if len(coursesAdmin)>0:
                coursesAdmin = coursesAdmin & coursesAdminAnterior
            else:
                coursesAdmin = coursesAdminAnterior
    ...
    return dict(visited = visited, coursesAdmin = coursesAdmin, countcoursesAdminT=countcoursesAdminT, coursesStudent=coursesStudent, coursesStudentT=coursesStudentT, split_name=split_name, split_section=split_section, 
periods=periods,period=period,periodo=periodo,periodo_year=db(db.period_year).select(), currentyear_period = cpfecys.current_year_period(),ecys_var=ecys_var,periods_var=periods_var, period_all = 
db(db.period).select(), coursesAdminAnterior=coursesAdminAnterior, period_detail=db(db.period_detail).select(), periodo_parametro= periodo_parametro)
...
def control_students_grades():
    ...
    if (auth.has_membership('Super-Administrator') == False and auth.has_membership('Ecys-Administrator') == False):
        ...
        if cpfecys.is_semestre(request.vars['year']):
            ...
            assigantion = db((db.user_project.assigned_user==auth.user.id)&\
                        (db.user_project.project == var_project)).select(db.user_project.ALL).first()
        ...
        if var_activity.laboratory == True or exception_s_var == True or var_activity.teacher_permition==True or var_activity.course_activity_category.teacher_permition==True:
            if var_activity.laboratory == True:
                ...
                if cpfecys.is_semestre(request.vars['year']):
                    ...
                    actualSemester = db(db.period_year.id==id_year).select().first()
                    controlP = db((db.period_detail.period==actualSemester.period)).select().first()
                pass
...
def student_control_period():
    ...
    grid = SQLFORM.grid(db.student_control_period, maxtextlength=100,csv=False,create=False,deletable=False,orderby=~db.student_control_period.id)
    return dict(grid=grid)
...
def control_weighting():
    ...
    if cpfecys.is_semestre(request.vars['year']):
        ...
        assigantion = db((db.user_project.assigned_user==auth.user.id)&\
                        (db.user_project.project == request.vars['project'])).select(db.user_project.ALL).first()
    #emarquez
    ...
    if auth.has_membership('Teacher'):
        ...
        if tipo_periodo:
            courses_request = db((db.user_project.assigned_user==auth.user.id)&\
                            (db.user_project.project==db.project.id)&\
                            (db.project.area_level==area.id)&\
                            (db.user_project.period == set_periodo)).select()
...
def General_report_activities():
    ...
    if cpfecys.is_semestre(request.vars['period']):             
        ...
        actualSemester = db(db.period_year.id==year.id).select().first()
        controlP = db((db.period_detail.period==actualSemester.period)).select().first()
...
def courses_avg():
    ...
    response.view='activity_control/courses_avg.html'
    if (auth.has_membership('Super-Administrator') or (auth.has_membership('Ecys-Administrator')& (ecys_var == True)) ):
        #coursesAdmin = db(db.project.area_level==area.id).select()
        coursesAdmin = []
        #for course in db(db.project.area_level==area.id).select(orderby=db.project.name):
    ...
    visited = db((db.page_visited.user_id == auth.user.id) & (db.page_visited.page_name == 'courses_avg')).select().first()
...
def graphs():
    ...
    esStudent = db((db.user_project.assigned_user == auth.user.id)&
                         (db.project.id == project_id)&
                         (db.user_project.project == db.project.id)).select().first()
    esAcademic = db((db.academic_course_assignation.carnet == db.academic.id)&
                    (db.academic_course_assignation.assignation == project_id)&
                    (db.academic.id_auth_user == auth.user.id)).select().first()
    
    if (esAcademic is not None) or (esStudent is not None) :
        ...
        ##Average of laboratory and period
        queryLab = 'select obtenerPromedio(\'T\',' + str(current_project.id) + ',' + str(currentyear_period.id) +') as promedio;'
        queryCurso = 'select obtenerPromedio(\'F\',' + str(current_project.id) + ',' + str(currentyear_period.id) +') as promedio;'
        avgLab = db.executesql(queryLab ,as_dict=True)
        avgCurso = db.executesql(queryCurso ,as_dict=True)
        varAvgLab = avgLab[0]['promedio']
        varAvgCurso = avgCurso[0]['promedio']
        
        if varAvgLab is None:
            varAvgLab = 0
        if varAvgCurso is None:
            varAvgCurso = 0
        
        avgL = [T("Laboratorio"), varAvgLab ]
        avgC = [T("Curso"), varAvgCurso ]
...
def GetProject():
    period=request.vars['period']
    return response.json(db((db.user_project.assigned_user==auth.user_id)&\
    (db.user_project.period==period)&\
    (db.period.id==db.period_detail.period)&\
    (db.user_project.project==db.project.id)&\
    (db.user_project.period==db.period_detail.period)).select())
